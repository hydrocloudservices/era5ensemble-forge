...
def list_available_data_not_in_bucket():
    ...
    all_combinations_filenames: list = ['{}_{}_ERA5_SL_ENS_REANALYSIS.nc'.format(date, variable.upper()) for
...
def save_unique_variable_date_file(dates_vars):
    ...
    for dayofmonth in pd.date_range(ds.time.values[0],ds.time.values[-1])[::-1]:
        ...
        for var in list(variables):
            filename = "{:04d}{:02d}{:02d}_{}_ERA5_SL_ENS_REANALYSIS.nc".format(dayofmonth.year,
                                                                                dayofmonth.month,
                                                                                dayofmonth.day,
                                                                                var.upper())
